...
void main()
{
    ...
    ButtonSetup();                              // Initialize the buttons
...
void TimerA0Setup()                             // Setup Timer 0
{
    ...
    TB0CCR1 = 1;                                // Set CCR1 to the lowest light level
}
...
void TimerA1Setup()                             // Setup Timer 1
{
    ...
    TB1CCR1 = 1;                                // Set CCR1 to the  lowest light level
}


#pragma vector=PORT2_VECTOR
__interrupt void Port_2(void)                   // Port 2 interrupt service routine
{
    P2IFG &= ~BIT3;                             //Clear the Flag
    if (TB0CCR1 >= 999)                         // If TB0CCR1 is less then or equal to 999
    TB0CCR1 = 1;                                // Set CCR1 to the lowest light level
    else                                        // If TB0CCR1 is not less then or equal to 999
        TB0CCR1 += 100;                         // Add 100 to TB0CCR1
}
...
{
    P4IFG &= ~BIT1;                             //Clear the Flag
    if (TB1CCR1 >= 999)                         // If TB1CCR1 is less then or equal to 999
    TB1CCR1 = 1;                                // Set CCR1 to the  lowest light level
    else                                        // If TB1CCR1 is not less then or equal to 999
        TB1CCR1 += 100;                         // Add 100 to TB1CCR1
}
